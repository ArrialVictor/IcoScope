...
def test_strip_hidden_until_time_field_selected(make_main_window):
    ...
    _set_field(win, 1, "precip")       # static
    assert not win._timeline_strip.isVisible()


def test_strip_shows_one_track_per_visible_pane(make_main_window):
    ...
    # Labels follow color_by.
    assert win._timeline_strip._tracks[0]._label == "tas_t"
    assert win._timeline_strip._tracks[1]._label == "tas_daily"
...
def test_strip_drag_propagates_cursor_to_panes(make_main_window):
...
def test_strip_domain_is_union_of_pane_axes(make_main_window):
    """Strip's shared domain spans min(first) → max(last) over visible panes."""
    win = make_main_window()
    win._on_pane_layout(2)
    QCoreApplication.processEvents()
    _set_field(win, 0, "tas_t")       # year-long
    _set_field(win, 1, "tas_daily")   # first 60 days
